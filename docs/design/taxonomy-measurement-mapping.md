...
- **Peg-variance check:** at period end, compare an oracle price to par. If
  `|price − 1.0| > threshold` (default 0.5%, configurable), set a
  `credit_review` flag on the holding and surface it in the remeasurement
  report. Stage 1 is **flag-only** — a full CECL reserve engine is out of scope
  (see §6).
...
- **Scope rule:** `digital_asset_type = 'wrapped-bridged-tokens'` resolves its
  treatment through an explicit, auditable **measurement election** per asset.
- **Two modes:**
...
- **Election is persisted** (`measurement_election`: asset, mode, rationale,
  ...
  was not marked to market.
- **Default:** `legal-claim` for volatile wrappers/bridges (xcDOT, xcASTR,
  WGLMR) unless a `technological-format` election is made. A **bridged
  stablecoin** (xcUSDT) resolves to §4.1 regardless — it is a redeemable-fiat
  claim first, a bridge second.

### 4.3 Network fees

- **Book:** unchanged — expensed to GL 5100. Make the functional class explicit:
  5100 is the default G&A-equivalent account; the mapping is configurable (a
  treasury vs. operating-entity choice). No code change is required to close the
  gate; the default is documented.
...
- **Unpriced holdings:** remeasurement already returns `status='partial'` when
  ...
  airdrops), aggregated and compared against a configurable materiality
  threshold (default 1% of total assets). A warning is emitted when the
  threshold is exceeded; the run stays `partial`.

## 5. Code changes implied

1. `remeasurement` scan: skip stablecoins (§4.1) and wrapped/bridged assets in
   `legal-claim` mode (§4.2).
2. A measurement-mode resolver keyed off `digital_asset_type` + the
   `measurement_election` table.
3. Peg-variance flag for stablecoins (§4.1) — flag-only.
...
6. Unpriced quarantine view + materiality config (§4.5).
...
## 7. Open decisions (board)

1. Wrapper default election — `legal-claim` vs `technological-format` (§4.2).
2. Peg-variance threshold (default 0.5%).
3. Unpriced-materiality threshold (default 1% of total assets).
4. Fee functional class default (G&A vs Cost of Revenue).

## 8. Test implications

Rust tests for: a stablecoin holding is not remeasured; a wrapper in each mode;
the proceeds waterfall choosing relinquished then received; `valuation_source`
being recorded; and the quarantine threshold. The existing proptest invariant
