...
> transaction figures that map to an identifiable individual are **redacted** in
> this public repository and kept only in private records. `[redacted …]`
> placeholders mark where a real address or exact figure appears in the private
> rehearsal copy. The rehearsal still uses the real wallets; the redaction
> applies to the public copy only.
...
| P2  | **Real wallet addresses with legitimate history** — at least one chain carrying an acquisition, a disposal, and ideally an own-wallet transfer.                     | Product owner | ✅ **Polkadot `[redacted Polkadot address]` verified active** — several hundred txs (2022 → 2025), staking / XCM / governance. ✅ **Ethereum `[redacted EVM address]` verified** — a small fixture wallet (a handful of transactions). ⚠️ **The real EVM history is on Moonbeam, which the app removed** — see finding #10. |
...
EVM `[redacted EVM address]` — a handful of real transactions, all on **Ethereum**:

| Time (UTC) | Direction | Detail                                      | Rehearsal role                    |
| ---------- | --------- | ------------------------------------------- | --------------------------------- |
| 16:20      | IN        | a small ETH deposit                            | acquisition                       |
| 20:34      | OUT       | ETH → a DEX router | disposal / swap                   |
| 20:46      | IN        | a small self-transfer                         | own-wallet transfer               |
| 20:56      | OUT       | a small ETH transfer out                 | disposal                          |
| —          | IN        | a stablecoin receipt                    | token receipt                     |
| —          | IN        | a token receipt                         | token receipt (verify legitimacy) |

Current ETH balance [redacted]. This satisfies checklist §3 step 5's acquisition / disposal / transfer requirement on Ethereum. Note the original address `[redacted EVM address]` (finding #6) remains useful as an unsolicited-airdrop/dust adversarial fixture.
...
| ------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2023-12 | 18 native + 9 ERC-20 = **27** | Several native inflows and outflows (an acquisition and two disposals); a native→wrapped wrap; a position opened and partly closed; a cross-chain stablecoin→token swap; and staking yield                                                                          | **Recommended.** Covers an acquisition, two disposals, a cross-asset measurement line, a swap, and yield — i.e. everything the statements need to exercise cost basis and realized gain/loss |
| 2026-01 | 6 native + 13 ERC-20 = **19** | All native rows are **zero-value** contract calls (token approvals; two to a precompile). All ERC-20 rows are **inbound** receipts of staking/reward tokens — no consideration paid and nothing leaving the wallet                                                   | **Not sufficient on its own.** Exercising neither acquisition nor disposal, it cannot rehearse the FIFO / realized-gain path the gate exists to test. Useful as a yield fixture only |
...
| 6   | §3 step 3 (import history)      | **Supplied EVM address carries no legitimate history.** `[redacted EVM address]` has zero native transactions on Ethereum and Arbitrum; its only activity is **dozens of inbound ERC-20 transfers, all unsolicited scam airdrops** (token names like "ACCESS [DOTFI.ORG] TO CLAIM", "stETH Visit www.weth.top to claim reward") sent from a single dusting address `[redacted source address]`, 2024 → 2025. No acquisition, disposal, or own-wallet transfer exists to rehearse.                                                                                                                                                                                                                                                                                                                                            | blocker (for the planned walkthrough) | **Superseded 2026-09-13** — a replacement EVM address with real activity was supplied and verified (see the pre-run data inventory). This original address is retained as an unsolicited-airdrop/dust adversarial fixture, which is itself worth rehearsing.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
