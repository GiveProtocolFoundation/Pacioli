...
| P2 | **Real wallet addresses with legitimate history** — at least one chain carrying an acquisition, a disposal, and ideally an own-wallet transfer. | Product owner | ✅ **Polkadot `13zaY…TAcF` verified active** — 287 txs, 2022-12 → 2025-10, nominationPools / xcmPallet / convictionVoting. ✅ **Ethereum `0x47bC…De81` verified** — clean 6-tx fixture (2026-09-12). ⚠️ **The real EVM history is on Moonbeam, which the app removed** — see finding #10. |
...
EVM `0x47bC8683b0D86296Cd596BfD2c670C0eD6D2De81` — 6 real transactions, all on **Ethereum**, all 2026-09-12:

| Time (UTC) | Direction | Detail | Rehearsal role |
| ---------- | --------- | ------ | -------------- |
| 16:20 | IN | 0.0103007883 ETH | acquisition |
| 20:34 | OUT | 0.005 ETH → `0x881d4023…` (MetaSwap router) | disposal / swap |
| 20:46 | IN | 0.001 ETH from self | own-wallet transfer |
| 20:56 | OUT | 0.001 ETH → `0x8cfc24ad1c…` | disposal |
| — | IN | 12.500709 mUSD (MetaMask USD, 6dp) | token receipt |
| — | IN | 5.0 ECX (eCash.com, 9dp) | token receipt (verify legitimacy) |

Current ETH balance 0.003702026213901276. This satisfies checklist §3 step 5's acquisition / disposal / transfer requirement on Ethereum. Note the original address `0x537f…aa75` (finding #6) remains useful as an unsolicited-airdrop/dust adversarial fixture.
...
1. **Run Gate 1 on Ethereum only.** The verified EVM data covers acquisition, disposal and an own-wallet transfer; Polkadot becomes a documented limitation of this gate. Lowest effort, and the honest position: the gate needs *real imported transactions*, not every chain.
